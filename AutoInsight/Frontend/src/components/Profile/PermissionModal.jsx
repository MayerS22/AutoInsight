...
const PermissionModal = ({ onClose, datasetId, uploaderId, currentUserPermission, sharedUsernames = [] }) => {
  ...
  // We'll always try to build full user details using sharedUsernames.
  ...
  // Available roles for adding – only allow "Owner" (admin) if current user is owner or admin.
  const availableRolesForAdding = [];
  if (currentUserPermission === "owner" || currentUserPermission === "admin") {
    availableRolesForAdding.push({ label: "Owner", value: "admin" });
  }
  availableRolesForAdding.push({ label: "Can view", value: "view" });
  availableRolesForAdding.push({ label: "Can edit", value: "edit" });

  useEffect(() => {
    const fetchPermissions = async () => {
      setIsFetchingPermissions(true);
      try {
        if (sharedUsernames && sharedUsernames.length > 0) {
          // For all roles, use sharedUsernames to fetch full user details via searchUsers.
            ...
            sharedUsernames.map(async (username) => {
              try {
                ...
                if (response.data.data && response.data.data.length > 0) {
                  ...
                  return {
                    _id: userDetail._id || `dummy-${username}`,
                  ...
                  return { _id: `dummy-${username}`, username, email: "Not available", profile_picture: null, access: "edit" };
                }
              } catch (error) {
          ...
          // Filter out the current user so they cannot edit their own role.
          const filteredUsers = detailedUsers.filter((u) => u.username !== currentUsername);
          setUsers(filteredUsers);
        } else {
          // Fallback: if no sharedUsernames available, try fetching via share endpoint.
    ...
    fetchPermissions();
  }, [datasetId, token, userId, sharedUsernames, currentUsername]);
  ...
  const handleAddUser = async () => {
    ...
    // Allow edit role to change any role now.
  ...
  const availableRolesForInline = [];
  if (currentUserPermission === "owner" || currentUserPermission === "admin") {
    availableRolesForInline.push({ label: "Owner", value: "admin" });
  }
  availableRolesForInline.push({ label: "Can view", value: "view" });
  availableRolesForInline.push({ label: "Can edit", value: "edit" });

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={onClose}>
      <div className="bg-white p-6 rounded-md relative w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mb-6">
            ...
            {suggestions.length > 0 && (
              <div className="absolute left-0 right-0 mt-1 bg-white border border-gray-300 rounded-md shadow-lg z-10">
                {suggestions.map((user) => (
                  <div key={user._id} className="flex items-center px-4 py-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleSelectUser(user)}>
                    <div className="flex-shrink-0 mr-3">
                      {user.profile_picture ? (
                        <img src={user.profile_picture} alt={`${user.username || "No Name"}'s profile`} className="w-10 h-10 rounded-full object-cover" />
                      ) : (
          ...
          <div className="relative">
            ...
            >
              <span>{selectedPermission === "admin" ? "Owner" : selectedPermission === "view" ? "Can view" : "Can edit"}</span>
              <ChevronDown size={16} />
            </button>
            {openRoleDropdown && (
              <div className="absolute right-0 mt-1 w-32 bg-purple-100 border border-gray-200 rounded-md shadow-lg z-10">
                {availableRolesForAdding.map((role) => (
                  <button key={role.value} onClick={() => { setSelectedPermission(role.value); setOpenRoleDropdown(false); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-purple-200">
                    {role.label}
                  </button>
                ))}
              </div>
            ...
            {users.map((user) => (
              <div key={user._id} className="flex items-center justify-between pb-4 border-b border-gray-300">
                <div className="flex items-center space-x-3">
                  {user.profile_picture ? (
                    <img src={user.profile_picture} alt={user.username || "No Name"} className="w-10 h-10 rounded-full object-cover" />
                  ) : (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    {updatingUserId === user._id ? (
                      ...
                      <button
                        ...
                        className="flex items-center justify-between bg-purple-100 border border-purple-300 rounded-md px-4 py-1 text-sm"
                      >
                        <span>{user.access === "admin" ? "Owner" : `Can ${user.access}`}</span>
                        <ChevronDown size={16} />
                      </button>
                    )}
                    {openDropdown === user._id && updatingUserId !== user._id && (
                      <div className="absolute right-0 mt-1 w-32 bg-purple-100 border border-gray-200 rounded-md shadow-lg z-10">
                        {availableRolesForInline.map((role) => (
                          <button key={role.value} onClick={() => { toggleAccess(user._id, role.value); setOpenDropdown(null); }} className="block w-full text-left px-4 py-2 text-sm hover:bg-purple-200">
                            {role.label}
                          </button>
                        ))}
                      </div>
                  ...
                  {(currentUserPermission === "owner" || currentUserPermission === "admin") && (
                    <button onClick={() => handleDeleteUserPermission(user._id)} disabled={deletingUserId === user._id} className="text-red-500 hover:text-red-700">
        ...
        <div className="flex justify-end">
          <button onClick={onClose} className="bg-purple-900 text-white px-8 py-2 rounded-md hover:bg-purple-800">
            Close
          </button>
        </div>
      </div>
